...
If you want to override the option and replace the current file contents with new
...
start outputting.
When you append output, if the file doesn't exist, UNIX creates it and writes the output.
...
$ more myStdOut      -rw-r- - r- - 1 username staff 1234  Oct 2 18:16 file1
$ more myStdErr
...
$ more myStdOut
Cannot open file noFile: No such file or directory   -rw-r- - r- - 1 username staff 1234  Oct   2 18:16 file1
```
...
We often need to use a series of commands to complete a task. For example if we need
...
Example:
```bash
$ cp file tempfile && echo   Copy successful
```
...
opening the file. It is as though the shell keeps the file in a buffer that provides instant access to
...
Total 6       -rw- - - - - - - 1 username staff 5782 10 16:19 TheRaven
```
...
+ ls –l fgLoop.scr file1 fileOut     -rwx- - - - - - 1 username staff 175 13 10:38 fgLoop.scr -rw-r - - r- - - 1 username staff 15 17 2000 file1 -rw-r - - r- - - 1 username staff 395 9 20:00 fileOut
```
...
Example:
```bash
% alias dir  echo Gil ergs Directory List; ls –l | more
% dir
```
...
$ HOME=$ (pwd)
$ echo $HOME
/mnt/diska/staff/username/unix13bash
...
CSH % setenv HOME /mnt/diska/staff/username
```
...
quotes. Because the space terminates an option, therefmore we must enclose it in quotes.
